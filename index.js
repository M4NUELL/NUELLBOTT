...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Manuel Ajg\n' 
            + 'ORG: Manuel Ajg;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6285854736710:+62 858-5473-6710\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
/*********** LOAD FILE ***********/
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST NUELBOT 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
       ...
       client.sendMessage(from, 'Tuh nomer pacarku❤️Eh owner ku >_<, jangan spam atau ku block kamu',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
           ...
           case 'demote':
            ...
            break
				...
				case 'clone':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Tag njing')
